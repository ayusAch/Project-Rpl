...
$query = "SELECT * FROM destinasi 
          INNER JOIN wisata ON destinasi.id_wisata = wisata.id_wisata 
          INNER JOIN lokasi ON destinasi.id_lokasi = lokasi.id_lokasi 
          INNER JOIN jadwal ON destinasi.id_jadwal = jadwal.id_jadwal
          INNER JOIN kuota ON destinasi.id_kuota = kuota.id_kuota
          ";
        ...
        <div class="inner_container">
                                        ...
                                        <form action="multi_delete.php" method="POST" id="deleteForm">
                                            <table class="table table-bordered">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $no = 1;
                                                    while ($dt_destinasi = $dt_query->fetch_assoc()) {
                                                        ?>
                                                        <tr>
                                                            <td><input type="checkbox" name="selected_ids[]"
                                                                    value="<?php echo ($dt_destinasi['id_destinasi']); ?>">
                                                            </td>
                                                            <td>
                                                                <?php echo ($no++); ?>
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['nama_wisata']); ?>
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['nama_destinasi']); ?>
                                                            </td>
                                                            <td><img src="../images/<?php echo $dt_destinasi['file']; ?>"
                                                                    alt="gambar" style="width: 150px ; height: auto ; ">
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['fasilitas']); ?>
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['jumlah_kuota']); ?>
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['nama_lokasi']); ?>
                                                            </td>
                                                            <td>
                                                                <?php echo ($dt_destinasi['hari']); ?>
                                                            </td>
                                                            <td>
                                                                <button type="button" class="btn btn-warning"
                                                                    onclick="showEditForm('<?php echo $dt_destinasi['id_destinasi']; ?>')">Edit</button>
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                                            ...
                                            <button type="submit" class="btn btn-danger"
                                                onclick="return confirm('Apakah Anda yakin ingin menghapus destinasi yang dipilih?')">Multi
                                                Delete</button>
                                        </form>
                                    </div>
                                </body>
            ...
            <script>
                ...
                function showEditForm(id) {
                    // Redirect ke halaman edit wisata dengan mengirimkan ID wisata sebagai parameter
                    window.location.href = "form_destinasi.php?action=edit&id=" + id;
                }
            </script>
</body>
