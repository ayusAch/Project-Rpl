...
$query = "SELECT * FROM registrasi WHERE level = 'user'";
$dt_query = $koneksi->query($query);
// query pengunjung
// $query_pengunjung = "SELECT * FROM pengunjung";
// $pengunjung = $koneksi->query($query_pengunjung);
?>
        ...
        <div class="inner_container">
                                    ...
                                    <div class="container mt-5">
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                <?php
                                                $no = 1;
                                                while ($dt_user = $dt_query->fetch_assoc()) {
                                                    ?>
                                                    <tr>
                                                        <td>
                                                            <?php echo ($no++); ?>
                                                        ...
                                                        <td>
                                                            <?php echo ($dt_user['email']); ?>
                                                        </td>
                                                        <td>
                                                            <?php echo ($dt_user['nama_lengkap']); ?>
                                                        </td>
                                                        <td>
                                                            <?php echo ($dt_user['alamat']); ?>
                                                        </td>
                                                        <td>
                                                            <?php echo ($dt_user['no_hp']); ?>
                                                        </td>
                                                        <td>
                                                            <?php echo ($dt_user['username']); ?>
                                                        </td>
                                                        <td>
                                                            <button type="button" class="btn btn-warning"
                                                                onclick="showEditForm('<?php echo $dt_user['id_registrasi']; ?>')">Edit</button>
                                                            <button type="button" class="btn btn-danger"
                                                                onclick="confirmDelete('<?php echo $dt_user['id_registrasi']; ?>')">Hapus</button>
                                                        </td>
                                                    </tr>
                                                    </tr>
                                                    <?php
            ...
            <script>
                ...
                function showEditForm(id) {
                    // Redirect ke halaman edit pengunjung dengan mengirimkan ID wisata sebagai parameter
                    window.location.href = "form_pengunjung.php?action=edit&id=" + id;
                }

                // Fungsi untuk menampilkan konfirmasi penghapusan data
                function confirmDelete(id) {
                    if (confirm("Apakah Anda yakin ingin menghapus data user ini?")) {
                        // Redirect ke halaman aksi_pengunjung.php dengan metode POST untuk menghapus data
                        var form = document.createElement("form");
                        form.setAttribute("method", "post");
                        form.setAttribute("action", "aksi_pengunjung.php");

                        // Buat input hidden untuk mengirimkan ID user
                        var hiddenId = document.createElement("input");
                        hiddenId.setAttribute("type", "hidden");
                        hiddenId.setAttribute("name", "id_registrasi");
