        ...
        <div class="inner_container">
                                ...
                                <body>
                                    <div class="container mt-5">
                                        <div class="d-flex justify-content-between mb-4">
                                            ...
                                            <a href='form_kuota.php' class="btn btn-success">Tambahkan Kuota</a>
                                        </div>
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                <?php
                                                $no = 1;
                                                while ($dt_kuota = $dt_query->fetch_assoc()) {
                                                    ?>
                                                    <tr>
                                                        <td>
                                                            <?php echo $no++; ?>
                                                        ...
                                                        <td>
                                                            <?php echo $dt_kuota['jumlah_kuota']; ?>
                                                        </td>
                                                        <td>
                                                            <?php echo $dt_kuota['keterangan']; ?>
                                                        </td>
                                                        <td>
                                                            <button type="button" class="btn btn-warning"
                                                                onclick="showEditForm('<?php echo $dt_kuota['id_kuota']; ?>')">Edit</button>
                                                            <button type="button" class="btn btn-danger"
                                                                onclick="confirmDelete('<?php echo $dt_kuota['id_kuota']; ?>')">Hapus</button>
                                                        </td>
                                                    </tr>
                                                    <?php
                                                }
            ...
            <script>
                ...
                function showEditForm(id) {
                    // Redirect ke halaman form_kuota.php dengan mengirimkan ID kuota sebagai parameter
                    window.location.href = "form_kuota.php?action=edit&id=" + id;
                }
                // Fungsi untuk menampilkan konfirmasi penghapusan data
                function confirmDelete(id) {
                    if (confirm("Apakah Anda yakin ingin menghapus kuota ini?")) {
                        // Redirect ke halaman aksi_kuota.php dengan metode POST untuk menghapus data
                        var form = document.createElement("form");
                        form.setAttribute("method", "post");
                        form.setAttribute("action", "aksi_kuota.php");

                        // Buat input hidden untuk mengirimkan ID wisata
                        var hiddenId = document.createElement("input");
                        hiddenId.setAttribute("type", "hidden");
                        hiddenId.setAttribute("name", "id_kuota");
